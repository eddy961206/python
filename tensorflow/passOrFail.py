...
model = tf.keras.models.Sequential([   # Sequential통해 레이어들 만들기
    tf.keras.layers.Dense(64, activation='tanh'),   # 레이어 하나에 노드 64개 만들기
    tf.keras.layers.Dense(128, activation='tanh'),  # 레이어나 노드의 갯수는 임의. 결과 잘 나올때까지 실험으로 파악해야.
    tf.keras.layers.Dense(1, activation='sigmoid') # 마지막 레이어의 노드는 확률을 한 개만 예측 할거면 노드가 하나여야.
    # 마지막 레이어의 활성함수는 sigmoid로. 그래야 확률을 0과 1 사이로 압축시켜서 알려줌. 물론 다른 많은 활성함수들도 있음.
])

model.compile(optimizer='adam',loss='binary_crossentropy',metrics=['accuracy'])  
                    # optimizer='adam' 이게 범용적으로 자주 쓰여. optimizer가 learning rate관련된 거임.
                    # 로스함수는 지금같은 경우 확률 0~1사이의 분류 문제면 binary_corssentropy쓰는게 좋아

# model.fit(학습데이터,실제정답,epochs=10)  epochs : 10번 학습시키기. 이 코드가 학습 하는 코드, 즉 최적의 w값 찾는 코드.
model.fit(np.array(xData), np.array(yData), epochs=1000)   #데이터들을 리스트 그대로 넣으면 안됨. numpy array로 변환해서 넣어야.

# 예측
predict_val = model.predict( [ [750, 3.70, 3], [400, 2.2, 1] ] )    # .predict(예측원하는 값) 여기선 두명의 성적 입력함.
print(predict_val)
